...
public class GridPositioner : MonoBehaviour
{
    ...
    {
        var dimension = new Vector2Int(rows, columns);
        return dimension;
    }

    public Transform GetHolderParent()
    {
        return transform;
    }
    
    public Vector3 GetLocalPosRelativeToHolder(Vector2Int gridPos)
    {
        Vector3 worldPos = transform.TransformPoint(GetLocalPos(gridPos));
        Vector3 localToHolder = m_HolderPos.InverseTransformPoint(worldPos);
        return localToHolder;
    }


    public Vector3 GetLocalPos(Vector2Int gridPos)
    {
